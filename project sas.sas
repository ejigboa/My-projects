...
LIBNAME PUF "C:\Users\mziyarnu\Documents\files\Masters application\GSU\SAS\final project\NISTEEN2018";
OPTIONS FMTSEARCH = (PUF WORK LIBRARY);

%LET MYPATH = C:\Users\mziyarnu\Documents\files\Masters application\GSU\SAS\final project\NISTEEN2018\RESULTS;
...
DATA MYPROJECT;
	SET PUF.NISTEENPUF18(KEEP = SEQNUMT PDAT2 PROVWT_C STRATUM YEAR AGE SEX RACEETHK EDUC1 AGEGRP_M_I 
						 MARITAL2 INCPORAR_I INS_STAT2_I CKUP_11_12 CEN_REG FACILITY STATE P_UTDMENACWY);
...
PROC FREQ DATA = PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
	WEIGHT PROVWT_C; /*** TO INCLUDE WEIGHTS FOR WEIGHTED RESULTS ***/
	TABLES (NEWAGE TEENAGEGP SEX MYRACEETH EDUC1 MARITAL2 AGEGRP_M_I MYINCTOPOV INS_STAT2_I MYCKUP1112 CEN_REG MYFACILITY)*P_UTDMENACWY / CHISQ;
	TITLE1 '2018 NIS-TEEN PUBLIC-USE FILE';
	TITLE2 'CHI-SQUARE TEST RESULTS FOR WEIGHTED VACCINATION COVERAGES BY SELECTED VARIABLES';
...
DATA MYCHISQTESTV1;
	...
	MYVALUE = PUT(VALUE, 10.2); /*** FORMATTING THE CHI-SQUARE STATISTIC TO 2 DECIMAL PLACES. ***/
...
PROC SGPLOT DATA=PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
    TITLE "Plotting Weighted Histogram Using Mother's age in NIS-Teen 2018 PUF";
...
PROC SGPLOT DATA=PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
    TITLE "Plotting Weighted Histogram Using income to proverty ratio in NIS-Teen 2018 PUF";
    HISTOGRAM MYINCTOPOV / BINSTART = 1 BINWIDTH = 1 NBINS = 5 WEIGHT = PROVWT_C;
    DENSITY MYINCTOPOV;
RUN;
...
PROC SGPLOT DATA=PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
    TITLE "Weighted Vertical Bar Chart Using Race/Ethnicity  in NIS-Teen 2018 PUF";
    VBAR MYRACEETH / WEIGHT = PROVWT_C STAT = PERCENT;
    FORMAT MYRACEETH FMTRACE.;
RUN;
PROC SGPLOT DATA=PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
    TITLE "Weighted Vertical Bar Chart Using Mother's educational level in NIS-Teen 2018 PUF";
...
PROC GCHART DATA=PROJECT;
   ...
   TITLE "Using the Medical insurance in NIS-Teen 2018 PUF";
   WHERE PDAT2 = 1  AND PROVWT_C NE .; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
...
PROC GCHART DATA=PROJECT;
   TITLE "Unweighted Pie-Chart";
   TITLE "Using the Metropolitan Statistical Area in NIS-Teen 2018 PUF";
   WHERE PDAT2 = 1  AND PROVWT_C NE .; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
...
PROC GCHART DATA=PROJECT;
   TITLE "Unweighted Pie-Chart";
   TITLE "Using the Vaccination facility type in NIS-Teen 2018 PUF";
   WHERE PDAT2 = 1  AND PROVWT_C NE .; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
...
PROC MEANS DATA=PROJECT MEAN CLM MAXDEC = 3;                                                       
    WHERE PDAT2 = 1 AND WEIGHT PROVWT_C NE .;                                                            
  	TITLE "  ";                                                                                    
   	CLASS ;                                                                                        
  	WEIGHT PROVWT_C;                                                                               
  	FREQ ;                                                                                         
...
%MYESTIMATE(NEWAGE, P_UTDMENACWY, MYAGE., 1, "Age (years) at Interview");
%MYESTIMATE(TEENAGEGP, P_UTDMENACWY, FMTAGRP., 2, "Age Group (in years) at Interview");
%MYESTIMATE(SEX, P_UTDMENACWY, FMTSEX., 3, "Sex of Adolescent at Interview");
%MYESTIMATE(MYRACEETH, P_UTDMENACWY, FMTRACE., 4, "Sex of Adolescent at Interview");
%MYESTIMATE(EDUC1, P_UTDMENACWY, FMTMEDU., 5, "Sex of Adolescent at Interview");
%MYESTIMATE(MARITAL2, P_UTDMENACWY, FMTMARRY., 6, "Sex of Adolescent at Interview");
%MYESTIMATE(AGEGRP_M_I, P_UTDMENACWY, FMTMAGRP., 7, "Sex of Adolescent at Interview");
%MYESTIMATE(MYINCTOPOV, P_UTDMENACWY, FMTINCPR., 8, "Sex of Adolescent at Interview");
%MYESTIMATE(INS_STAT2_I, P_UTDMENACWY, FMTINS., 9, "Sex of Adolescent at Interview");
%MYESTIMATE(MYCKUP1112, P_UTDMENACWY, FMTCKUP., 10, "Sex of Adolescent at Interview");
%MYESTIMATE(CEN_REG, P_UTDMENACWY, FMTCENREG., 11, "Sex of Adolescent at Interview");
%MYESTIMATE(MYFACILITY, P_UTDMENACWY, FMTFACTY., 12, "Sex of Adolescent at Interview");
%MYESTIMATE(STATE, P_UTDMENACWY, FMSTATE., 13, "Sex of Adolescent at Interview");
...
PROC SURVEYFREQ DATA = PROJECT;
    ...
    TABLES  PDAT2*(P_UTDMENACWY NEWAGE SEX MYRACEETH EDUC1 MARITAL2 AGEGRP_M_I MYINCTOPOV INS_STAT2_I MYCKUP1112 CEN_REG MYFACILITY)/ CL (TYPE=CP) NOWT NOSTD;
RUN;
...
DATA FINALSTABLE1V1;
	...
	SET FINALSTABLE1(KEEP = AGEGRP_M_I CEN_REG EDUC1 FREQUENCY INS_STAT2_I LOWERCL MARITAL2 MYCKUP1112 MYFACILITY MYINCTOPOV 
					 MYRACEETH NEWAGE PDAT2 P_UTDMENACWY PERCENT SEX TABLE UPPERCL _SKIPLINE);
...
DATA FINALSTABLE1V2;
	...
	/*** MYVARIABLE = THE LABEL FOR EACH VALUE OF VARIABLE; MYVARCOUNT = THE VARIABLE NUMBER IN THE TABLE; MYVARNUM = THE VALUE OF EACH VARIABLE ***/
	IF P_UTDMENACWY NE . THEN DO;
		MYVARIABLE = PUT(P_UTDMENACWY, FMTUTD.);
		MYVARCOUNT = 0;
		MYVARNUM = P_UTDMENACWY;
	END;
    IF NEWAGE NE . THEN DO;
		MYVARIABLE = PUT(NEWAGE, MYAGE.);
		MYVARCOUNT = 1;
...
%MYESTIMATES(P_UTDMENACWY, NEWAGE, MYAGE., 1, "AGE IN YEARS OF SELECTED TEEN");
%MYESTIMATES(P_UTDMENACWY, SEX, FMTSEX., 2, "SEX OF TEEN");
%MYESTIMATES(P_UTDMENACWY, MYRACEETH, FMTRACE., 3, "RACE/ETHNICITY OF TEEN WITH MULTIRACE CATEGORY (RECODE)");
%MYESTIMATES(P_UTDMENACWY, EDUC1, FMTMEDU., 4, "EDUCATION LEVEL OF MOTHER WITH 4 CATEGORIES (RECODE)");
%MYESTIMATES(P_UTDMENACWY, MARITAL2, FMTMARRY., 5, "MARITAL STATUS OF MOTHER (RECODE)");
%MYESTIMATES(P_UTDMENACWY, AGEGRP_M_I, FMTMAGRP., 6, "MOTHER'S AGE CATEGORIES (RECODE)");
%MYESTIMATES(P_UTDMENACWY, MYINCTOPOV, FMTINCPR., 7, "(INCOME TO POVERTY RATIO: IMPUTED (RECODE)");
%MYESTIMATES(P_UTDMENACWY, INS_STAT2_I, FMTINS., 8, "INSURANCE STATUS (PRIVATE ONLY/ANY MEDICAID/OTHER INSURANCE/UNINSURED): IMPUTED");
%MYESTIMATES(P_UTDMENACWY, MYCKUP1112, FMTCKUP., 9, "DID TEEN HAVE AN 11-12 YEAR OLD WELL-CHILD EXAM OR CHECK-UP?");
%MYESTIMATES(P_UTDMENACWY, CEN_REG, FMTCENREG., 10, "CENSUS REGION BASED ON TRUE STATE OF RESIDENCE");
%MYESTIMATES(P_UTDMENACWY, MYFACILITY, FMTFACTY., 11, "FACILITY TYPES FOR TEEN'S PROVIDERS");


DATA PUF.FINALTABLE2;
	SET FINALTABLE2;
RUN;
PROC EXPORT DATA = FINALTABLE2 OUTFILE= "&MYPATH.\TABLE2\FINALTABLE2.xlsx"
	DBMS=XLSX REPLACE;
RUN;
...
PROC SURVEYLOGISTIC DATA = PROJECT;
    ...
    MODEL  P_UTDMENACWY(EVENT = "1") = &VAR1.;
...
DATA MYORESTV1&VAR1.;
	...
	LENGTH MYRESULTS NEWEFFECT $25.;
	SET MYOREST&VAR1.;
	WHERE PDAT2 = 1;

	NEWEFFECT = EFFECT;
	
	MYESTMATE = PUT(ROUND(ODDSRATIOEST,0.00001), 8.5);
	CONINTEL = '('||PUT(ROUND(LOWERCL,0.00001), 8.5)||' - '||PUT(ROUND(UPPERCL,0.00001), 8.5)||')';
	CONINTELFL = COMPRESS(CONINTEL);
...
DATA MYTYPE3TESTV1&VAR1.;
	...
	LENGTH MYVARIABLE NEWEFFECT $25.;
	...
	KEEP MYVARCOUNT NEWEFFECT MYVARIABLE MYWALDCHISQ MYPROBCHISQ;
RUN;
...
PROC EXPORT DATA = ODDSRATIOSTABLE3 OUTFILE ="&MYPATH.\TABLE2\ODDSRATIOSTABLE3.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYUNADJPARATTEST OUTFILE ="&MYPATH.\TABLE3\MYUNADJPARATTESTTABLE3.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYTYPE3TEST OUTFILE ="&MYPATH.\TABLE3\MYTYPE3TESTTABLE3.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC SURVEYLOGISTIC DATA = PROJECT;
    ...
    MODEL  P_UTDMENACWY(EVENT = "1") = NEWAGE SEX MYRACEETH EDUC1 MARITAL2 AGEGRP_M_I MYINCTOPOV INS_STAT2_I MYCKUP1112 CEN_REG MYFACILITY;
...
PROC EXPORT DATA = MYADJORESTV2 OUTFILE ="&MYPATH.\TABLE2\MULTILOGODDSRATIOTABLE3.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYADJTYPE3TESTV1 OUTFILE ="&MYPATH.\TABLE3\MULTILOGTYPE3TESTTABLE3.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC SURVEYLOGISTIC DATA = PROJECT;
    ...
    WEIGHT PROVWT_C; /*** FINAL DUAL-FRAME PROVIDER-PHASE WEIGHT (EXCLUDES TERRITORIES) ***/
	DOMAIN PDAT2;    /*** ADEQUATE PROVIDER DATA FLAG ***/
	CLASS MYRACEETH(PARAM=REF REF="1")MYCKUP1112(PARAM=REF REF="1") CEN_REG(PARAM=REF REF="1") MYFACILITY(PARAM=REF REF="1");

    MODEL  P_UTDMENACWY(EVENT = "1") =  MYRACEETH MYCKUP1112 CEN_REG MYFACILITY;
...
PROC EXPORT DATA = MYORESTML4V2 OUTFILE ="&MYPATH.\TABLE2\MULTILOGODDSRATIOTABLE3ML4.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYADJPARAESTML4V1 OUTFILE ="&MYPATH.\TABLE3\MULTILOGTTESTTABLE3ML4.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYTYPE3TESTML4V1 OUTFILE ="&MYPATH.\TABLE3\MULTILOGTYPE3TESTTABLE3ML4.xlsx" DBMS=XLSX REPLACE;
RUN;
